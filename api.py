...
def place_order(product_index: int, buyer: str, payment_mode: str = "PREPAID"):
    ...
    seller_deposit = int(product["price"] * 0.10) if normalized_mode == "COD" else 0
    ...
    order = {
        ...
        "seller_security_deposit": seller_deposit,
...
def mark_delivered(order_id: int):
    ...
    if deposit > 0 and seller_user["escrow_wallet"] >= deposit:
        seller_user["escrow_wallet"] -= deposit
        seller_user["regular_wallet"] += deposit
...
def _apply_return_rejection_penalty(order):
    ...
    penalty_amount = int(price * 0.10)
    seller_share = int(penalty_amount * 0.70)
    agent_share = penalty_amount - seller_share
    refund_amount = 0

    if payment_mode == "PREPAID":
        refund_amount = int(price * 0.90)
        seller_deduction = price - seller_share
        if seller_user["regular_wallet"] < seller_deduction:
            raise HTTPException(status_code=400, detail="Seller does not have enough funds to complete the return penalty")

        seller_user["regular_wallet"] -= seller_deduction
        buyer_user["regular_wallet"] += refund_amount
        agent_user["service_fee_wallet"] += agent_share
    else:
        if buyer_user["escrow_wallet"] >= penalty_amount:
            buyer_user["escrow_wallet"] -= penalty_amount
        elif buyer_user["regular_wallet"] >= penalty_amount:
            buyer_user["regular_wallet"] -= penalty_amount
        else:
            raise HTTPException(status_code=400, detail="Buyer does not have enough funds for penalty")
...
def _apply_return_approval_refund(order):
    buyer_user = _get_user(order.get("buyer", ""))
    seller_user = _get_user(order.get("seller", ""))
    price = int(order["product"]["price"])
    payment_mode = str(order.get("payment_mode", "PREPAID")).upper()
    ...
    if payment_mode == "PREPAID":
        if seller_user["regular_wallet"] < price:
            raise HTTPException(status_code=400, detail="Seller does not have enough funds to refund the prepaid order")
        seller_user["regular_wallet"] -= price
        buyer_user["regular_wallet"] += price
        return {
            "refund_amount": price,
    ...
    return {"refund_amount": 0, "payment_mode": payment_mode}
...
def agent_review_return(order_id: int, decision: str):
    ...
    if normalized_decision == "REASONABLE":
        ...
        result = _apply_return_approval_refund(order)
        result["status"] = "return_reviewed"
        result["decision"] = normalized_decision
    else:
        _decrease_reputation(order.get("buyer", ""), 0.1)
        order["status"] = "RETURN_REJECTED"
        result = _apply_return_rejection_penalty(order)
        result["status"] = "return_reviewed"
        result["decision"] = normalized_decision

    order["return_decision"] = normalized_decision
    order["agent_notified"] = False
    result["buyer_rep"] = buyer_user.get("rep", 0)
    result["seller_rep"] = seller_user.get("rep", 0)
    return result
...
def get_user_state(username: str):
    ...
    return {
        "username": username,
        "role": user.get("role"),
        "regular_wallet": 0 if is_agent else user.get("regular_wallet", 0),
        "escrow_wallet": 0 if is_agent else user.get("escrow_wallet", 0),
        "service_fee_wallet": user.get("service_fee_wallet", 0) if is_agent else 0,
